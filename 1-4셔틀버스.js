...
for (let i = 0; i < n; i++) {
    ...
    if (i == (n - 1)) {         // 마지막 버스인 경우
      if (currentPeople >= m) { // 타야하는 승객수가 m이상인 경우 m-1에 나와 무조건 버스를 탐
        currentBusTime = times[m - 1] - 1;
      }
    } else {                    // 마지막 버스가 아닌 경우는 기존에 크루들을 태움
      if (currentPeople > m) {
        times.splice(0, m);
      }else if (currentPeople <= m) {
...
answer = String(hour < 10 ? "0" + hour : hour) +  ":" +  String(min < 10 ? "0" + min : min);
